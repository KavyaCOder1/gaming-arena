...
import { ArrowLeft, Trophy, Crown, Medal, Zap, LayoutGrid, Grid3X3, Timer, User } from "lucide-react";
...
const DIFF_CONFIG = {
  EASY:   { label: "ALPHA",  desc: "4×4 Grid · Slow Flip",  color: "#10b981", bg: "rgba(16,185,129,0.12)",  border: "rgba(16,185,129,0.4)",  glow: "rgba(16,185,129,0.25)",  xp: "200 XP" },
  MEDIUM: { label: "BETA",   desc: "6×6 Grid · Fast Flip",  color: "#f59e0b", bg: "rgba(245,158,11,0.12)", border: "rgba(245,158,11,0.4)",  glow: "rgba(245,158,11,0.25)",  xp: "350 XP" },
  HARD:   { label: "OMEGA",  desc: "8×8 Grid · Hyper Flip", color: "#ef4444", bg: "rgba(239,68,68,0.12)",  border: "rgba(239,68,68,0.4)",   glow: "rgba(239,68,68,0.25)",   xp: "500 XP" },
};
...
export default function MemoryPage() {
  ...
  useEffect(() => {
    (async () => {
      setLbLoading(true);
      try {
        const res = await fetch("/api/leaderboard?gameType=MEMORY");
    ...
    <div style={{ width: "100%", minHeight: "100vh", paddingBottom: 40, boxSizing: "border-box" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            <h1 style={{ fontFamily: "'Orbitron',sans-serif", fontSize: "clamp(26px,5vw,50px)", fontWeight: 900, color: C.text, textTransform: "uppercase", fontStyle: "italic", letterSpacing: "-0.02em", lineHeight: 1, margin: 0 }}>
              MEMORY <span style={{ color: C.cyan, textShadow: "0 0 20px rgba(34,211,238,0.5)" }}>MATRIX</span>
            </h1>
            <p style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 11, fontWeight: 600, color: "#334155", letterSpacing: "0.25em", textTransform: "uppercase", marginTop: 5 }}>
              PATTERN ENGINE · v4.2 Cognitive Sync
            </p>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 8, color: "#334155", letterSpacing: "0.25em", textTransform: "uppercase", marginBottom: 3 }}>Speed Bonus</div>
            <div style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 26, fontWeight: 900, color: "#f59e0b", filter: "drop-shadow(0 0 10px rgba(245,158,11,0.4))" }}>2× XP</div>
          </div>
        </div>
      </div>

      {/* ── MAIN LAYOUT ── */}
      <div className="memory-layout" style={{ display: "flex", gap: 18, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ position: "relative", borderRadius: 22, overflow: "hidden", border: "1px solid rgba(34,211,238,0.15)", boxShadow: "0 0 0 1px rgba(99,102,241,0.08), 0 8px 40px rgba(0,0,0,0.5)" }}>
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
              <img
                alt="Memory Preview"
                src="https://images.unsplash.com/photo-1611996575749-79a3a250f968?q=80&w=1000&auto=format&fit=crop"
              ...
              <div style={{ position: "absolute", top: 20, left: 20 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(12px)", padding: "7px 14px", borderRadius: 10, border: "1px solid rgba(34,211,238,0.4)" }}>
                  <span style={{ width: 8, height: 8, borderRadius: "50%", background: C.cyan, boxShadow: `0 0 10px ${C.cyan}`, display: "inline-block", animation: "memLive 2s infinite" }} />
                  <span style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 9, fontWeight: 700, color: "#fff", letterSpacing: "0.2em", textTransform: "uppercase" }}>Neural Sync Ready</span>
                </div>
              </div>
              {/* bottom info bar */}
              <div style={{ position: "absolute", bottom: 20, left: 20, right: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ padding: 14, background: "rgba(15,23,42,0.85)", borderRadius: 16, border: "1px solid rgba(99,102,241,0.3)" }}>
                    <LayoutGrid style={{ width: 28, height: 28, color: C.indigo, filter: "drop-shadow(0 0 10px rgba(99,102,241,0.7))" }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 12, fontWeight: 900, color: "#fff", letterSpacing: "0.08em" }}>PATTERN ENGINE</div>
                    <div style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, fontWeight: 700, color: C.muted, letterSpacing: "0.25em", marginTop: 2 }}>v4.2 COGNITIVE SYNC</div>
                  </div>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  {[LayoutGrid, Grid3X3].map((Icon, i) => (
          ...
          <div style={{ ...card, padding: "20px" }}>
            <div style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 8, fontWeight: 700, color: "#334155", letterSpacing: "0.3em", textTransform: "uppercase", marginBottom: 14 }}>Select Neural Load</div>
          ...
          <div style={{ ...card, padding: "16px 20px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 16 }}>
              {[
                { key: "MOUSE CLICK", action: "Flip Tile" },
                { key: "T KEY",       action: "Toggle Theme" },
                { key: "~1 MIN",      action: "Avg Round" },
              ].map((item, i) => (
        ...
        <div className="memory-right" style={{ width: 260, flexShrink: 0, display: "flex", flexDirection: "column", gap: 14 }}>

          {/* Mode info badge */}
          <div style={{ ...card, padding: "16px 18px", borderColor: cfg.border.replace("0.4)", "0.25)"), boxShadow: `0 0 18px ${cfg.glow}` }}>
            <div style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 7, color: C.muted, letterSpacing: "0.25em", textTransform: "uppercase", marginBottom: 6 }}>SELECTED LOAD</div>
            ...
            <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 8, color: C.muted, letterSpacing: "0.15em", textTransform: "uppercase" }}>XP Reward</span>
          ...
          <div style={{ ...card, padding: 18, flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 9, fontWeight: 700, color: C.text, letterSpacing: "0.2em", textTransform: "uppercase" }}>MIND MASTERS</span>
              <div style={{ marginLeft: "auto", padding: "2px 8px", borderRadius: 6, background: "rgba(99,102,241,0.12)", border: "1px solid rgba(99,102,241,0.25)", fontFamily: "'Orbitron',sans-serif", fontSize: 7, fontWeight: 900, color: C.indigo, letterSpacing: "0.12em" }}>TOP TIMES</div>
            </div>

            {lbLoading ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                {[...Array(5)].map((_, i) => <div key={i} style={{ height: 36, borderRadius: 9, background: "rgba(255,255,255,0.03)", animation: "memSkel 1.5s infinite" }} />)}
              </div>
            ...
            {user && (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 10, background: "rgba(99,102,241,0.08)", border: "1px solid rgba(99,102,241,0.2)" }}>
                  ...
                  <div style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 9, fontWeight: 700, color: C.muted }}>RANK —</div>
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[
              { label: "Avg Round", value: "~1 MIN", color: C.cyan },
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, width: "100%", padding: "0 48px 24px", pointerEvents: "none", zIndex: -1, opacity: 0.04 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div style={{ fontFamily: "'Orbitron',sans-serif", fontSize: "clamp(60px,12vw,120px)", lineHeight: 1, fontWeight: 900, color: C.text, userSelect: "none", fontStyle: "italic" }}>MEMORY</div>
      ...
      <style>{`
        @keyframes memLive { 0%,100%{opacity:1} 50%{opacity:0.35} }
        @keyframes memSkel { 0%,100%{opacity:1} 50%{opacity:0.35} }

        /* Responsive: stack on smaller screens */
        .memory-layout { flex-direction: row; }
        @media(max-width:900px) {
          .memory-layout { flex-direction: column !important; }
          .memory-right { width: 100% !important; }
        }
      `}</style>
    </div>
  );
