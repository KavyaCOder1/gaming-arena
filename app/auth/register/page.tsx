...
import { Loader2, Gamepad2, AlertCircle, Eye, EyeOff, Lock, User, Shield } from "lucide-react";
        ...
        <div style={{ width: "100%", maxWidth: 440 }}>
            <div style={{ background: C.dark, backdropFilter: "blur(28px)", WebkitBackdropFilter: "blur(28px)", border: `1px solid ${C.border}`, borderRadius: 28, padding: "40px 36px", boxShadow: "0 0 60px rgba(0,0,0,0.8)", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ textAlign: "center", marginBottom: 32 }}>
                    <div style={{ display: "inline-flex", padding: 14, borderRadius: 18, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.25)", marginBottom: 16 }}>
                        <Gamepad2 style={{ width: 32, height: 32, color: C.indigo }} />
                    </div>
                    <h1 style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 26, fontWeight: 900, color: C.text, textTransform: "uppercase", letterSpacing: "-0.02em", marginBottom: 6 }}>
                        CREATE <span style={{ color: C.cyan }}>ACCOUNT</span>
                    </h1>
                    <p style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 11, fontWeight: 700, color: C.muted, letterSpacing: "0.25em", textTransform: "uppercase" }}>Join Gaming Arena</p>
                </div>
                ...
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                    ...
                    <div>
                        ...
                        <div style={{ position: "relative" }}>
                            ...
                            <input type="text" required value={username} onChange={e => setUsername(e.target.value)} placeholder="Choose a username" style={inputBase}
                        ...
                        {password.length > 0 && (
                            <div style={{ marginTop: 8 }}>
                                <div style={{ display: "flex", gap: 4, marginBottom: 4 }}>
                                    {[1, 2, 3].map(lvl => (
                                        <div key={lvl} style={{ flex: 1, height: 3, borderRadius: 2, background: pwStrength >= lvl ? pwColors[pwStrength] : "rgba(30,41,59,0.8)", transition: "all 0.3s" }} />
                                    ))}
                                </div>
                                <span style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 8, letterSpacing: "0.2em", color: pwColors[pwStrength], textTransform: "uppercase" }}>
                                    {pwLabels[pwStrength]}
                                </span>
                            </div>
                        )}
                    ...
                    <div>
                        ...
                        <div style={{ position: "relative" }}>
                            <Lock style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", width: 16, height: 16, color: "rgba(34,211,238,0.4)", pointerEvents: "none" }} />
                            <input type="password" required value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="Repeat password" style={inputBase}
                                onFocus={e => { e.target.style.borderColor = "rgba(34,211,238,0.6)"; e.target.style.boxShadow = "0 0 14px rgba(34,211,238,0.15)"; }}
                                onBlur={e => { e.target.style.borderColor = C.border; e.target.style.boxShadow = "none"; }} />
                        </div>
                    </div>
                ...
                <div style={{ marginTop: 24, paddingTop: 20, borderTop: "1px solid rgba(30,41,59,0.7)", textAlign: "center" }}>
                    ...
                    <Link href="/auth/login" style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 9, fontWeight: 700, color: C.cyan, letterSpacing: "0.2em", textTransform: "uppercase", textDecoration: "underline", textUnderlineOffset: 4 }}>LOGIN</Link>
                </div>
            </div>
