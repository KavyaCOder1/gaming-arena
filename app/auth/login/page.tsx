            ...
            <div style={{ background: C.dark, backdropFilter: "blur(28px)", WebkitBackdropFilter: "blur(28px)", border: `1px solid ${C.border}`, borderRadius: 28, padding: "40px 36px", boxShadow: "0 0 60px rgba(0,0,0,0.8), 0 0 40px rgba(34,211,238,0.06)", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ textAlign: "center", marginBottom: 32 }}>
                    <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.05 }}
                        style={{ display: "inline-flex", padding: 14, borderRadius: 18, background: "rgba(34,211,238,0.1)", border: "1px solid rgba(34,211,238,0.25)", boxShadow: "0 0 24px rgba(34,211,238,0.18)", marginBottom: 16 }}>
                        <Gamepad2 style={{ width: 32, height: 32, color: C.cyan, filter: "drop-shadow(0 0 8px rgba(34,211,238,0.7))" }} />
                    </motion.div>
                    <h1 style={{ fontFamily: "'Orbitron',sans-serif", fontSize: "clamp(22px,5vw,30px)", fontWeight: 900, color: C.text, textTransform: "uppercase", fontStyle: "italic", letterSpacing: "-0.02em", marginBottom: 8 }}>
                        USER <span style={{ color: C.cyan }}>LOGIN</span>
                    </h1>
                    <p style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 11, fontWeight: 700, color: C.muted, letterSpacing: "0.25em", textTransform: "uppercase" }}>Access your operative account</p>
                </div>
                ...
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                    ...
                    <div>
                        ...
                        <div style={{ position: "relative" }}>
                            ...
                            <input type="text" required value={username} onChange={e => setUsername(e.target.value)} placeholder="Enter username"
                                style={{ width: "100%", boxSizing: "border-box", padding: "14px 14px 14px 44px", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, color: C.text, fontFamily: "'Rajdhani',sans-serif", fontSize: 15, fontWeight: 600, outline: "none", transition: "all 0.2s" }}
                    ...
                    <div>
                        ...
                        <div style={{ position: "relative" }}>
                            ...
                            <input type={showPassword ? "text" : "password"} required value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••"
                                style={{ width: "100%", boxSizing: "border-box", padding: "14px 44px 14px 44px", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, color: C.text, fontFamily: "'Rajdhani',sans-serif", fontSize: 15, fontWeight: 600, outline: "none", transition: "all 0.2s", letterSpacing: showPassword ? "normal" : "0.1em" }}
                                onFocus={e => { e.target.style.borderColor = "rgba(34,211,238,0.6)"; e.target.style.boxShadow = "0 0 14px rgba(34,211,238,0.15)"; }}
                                onBlur={e => { e.target.style.borderColor = C.border; e.target.style.boxShadow = "none"; }} />
                    ...
                    <motion.button type="submit" disabled={loading} whileHover={!loading ? { scale: 1.02, y: -2 } : {}} whileTap={!loading ? { scale: 0.97 } : {}}
                        ...
                        <span style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                            {loading ? <><Loader2 style={{ width: 18, height: 18, animation: "spin 1s linear infinite" }} />LOGGING IN...</> : <><Zap style={{ width: 16, height: 16 }} />LOG IN</>}
                        </span>
                    </motion.button>
                </form>

                <div style={{ marginTop: 24, paddingTop: 20, borderTop: "1px solid rgba(30,41,59,0.7)", textAlign: "center" }}>
                    <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 13, color: C.muted, fontWeight: 600 }}>Don't have an account? </span>
                    <Link href="/auth/register" style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 9, fontWeight: 700, color: C.cyan, letterSpacing: "0.2em", textTransform: "uppercase", textDecoration: "underline", textUnderlineOffset: 4 }}>REGISTER HERE</Link>
                </div>
